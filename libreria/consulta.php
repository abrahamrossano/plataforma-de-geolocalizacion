...
class consulta{
    ...
    function getInfoCho($usu){
		$query = sprintf("SELECT * FROM chofer where baja = false AND id_usuario1 = %s ",
    ...
    function getFullInfoUsuario($usu){
		$query = sprintf("SELECT * FROM usuario where baja = false AND id_usuario = %s ",
		GetSQLValueString($usu,"text"));
		$this->sql->setQuery($query);
		$res=$this->sql->loadMatrixAssoc();
		if ($this->sql->errorNum==0)
